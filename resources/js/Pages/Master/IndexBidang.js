...
export default function Index() {
  ...
  const edit = (id) => {
    Inertia.get(route('master.bidang.edit', id));
  }

  const destroy = (id) => {
    delData().then((result) => {
      if (result.isConfirmed) {
        Inertia.delete(route('master.bidang.destroy', id));
      }
    })
  }
    ...
    <div className="py-6 px-4 sm:p-6 lg:pb-8">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div className="flex-1 min-w-0 py-2">
          <h2 className="text-lg leading-6 font-medium text-gray-900">Bidang</h2>
        </div>
        <div className="mt-4 flex sm:mt-0 sm:ml-4">
          <Link
            className="px-4 py-2 sm:order-1 sm:ml-3 btn-primary"
            href={route('master.bidang.create')}
          >
...
Index.layout = page => <Layout title="Master Bidang" children={page} />;
