...
export default function Index() {
  ...
  const getRows = () => {
    if (rowt > ticker.length) rowt = 1;

    return rowt++;
  };

  const edit = (id) => {
    Inertia.get(route("ticker.edit", id));
  };

  const destroy = (id) => {
    delData().then((result) => {
      if (result.isConfirmed) {
        Inertia.delete(route("ticker.destroy", id));
      }
    });
  };
...
Index.layout = (page) => <Layout title="Ticker" children={page} />;
